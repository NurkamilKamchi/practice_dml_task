...
-- Сгруппировать людей по полу и рассчитать средний возраст для каждого пола
    select gender,avg(people.age) from people group by  gender;
-- Вывести имя в год рождение людей которые говорят на английском языке
    select first_name, people.age from people where language ='Kazakh';
...
-- Вывести фамилии, почту и айди всех женщин
    select people.last_name,people.email,people.id from people where gender = 'Female';
-- Вывести имя и фамилие вместе переименовав как full_name(concat as), пол и год рождение всех мужчин родившихся после 2000 года
    select people.gender,people.age,concat(people.first_name ,' ', people.last_name) as full_name from people where age>2000;
...
select  * from people offset 20 rows fetch first 5 rows only;
--#19 Подсчитать количество людей для каждого языка
    select count(first_name), language  from people   group by language ;
-- Найти средний возраст для каждого языка, где количество людей больше 2
    select language,count(id)>2,avg(people.age) from people group by language ;
-- Вывести количество записей где почта закончивается на .com
    select count(people.email)from people where email like '%.com';
--#21 Найти самый распространенный язык среди женщин
    select language from people where gender ilike 'female' order by people.language desc limit 1;
-- Обновить язык для людей, чей возраст между 3 и 10 годами, на 'French'
    update people set language= 'French' where age between 2014 and 2021;
select people.first_name,people.language from people where language = 'french';

-- select first_name,people.language from people  where age between 3 and 10;
